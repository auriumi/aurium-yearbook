// src/components/registration/AddressStep.tsx
  ...
  <div className="space-y-4">
    <div className="space-y-2">
        ...
        <Select value={selectedProvinceCode} onValueChange={handleProvinceChange}>
            ...
            <SelectContent className="max-h-[200px]">{provinceList.map((prov: any) => (<SelectItem key={prov.code} value={String(prov.code)}>{prov.name}</SelectItem>))}</SelectContent>
        </Select>
    </div>
    <div className="space-y-2">
        ...
        <Select value={selectedCityCode} onValueChange={handleCityChange} disabled={!selectedProvinceCode}>
            ...
            <SelectContent className="max-h-[200px]">{cityList.map((city: any) => (<SelectItem key={city.code} value={String(city.code)}>{city.name}</SelectItem>))}</SelectContent>
        </Select>
    </div>
    <div className="space-y-2">
        ...
        <Select value={selectedBarangayCode} onValueChange={setSelectedBarangayCode} disabled={!selectedCityCode}>
            ...
            <SelectContent className="max-h-[200px]">{barangayList.map((brgy: any) => (<SelectItem key={brgy.code} value={String(brgy.code)}>{brgy.name}</SelectItem>))}</SelectContent>
        </Select>
    </div>
  </div>
);